    ...
    for obstacle_position in obstacle_positions:
        ...
        for di, dj in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
            ...
            if 0 <= new_i < len(board) and len(board[0]) > new_j >= 0 == board[new_i][new_j]:
                openness = sum(1 for ddi in [-1, 0, 1] for ddj in [-1, 0, 1] if
                               0 <= new_i + ddi < len(board) and len(board[0]) > new_j + ddj >= 0 == board[new_i + ddi][new_j + ddj])
                if openness > 6:
                    return new_i, new_j
...
def InitPos(mapStat):
    ...
    '''
    mapStat = np.asarray(mapStat, dtype=int)
    # print(mapStat.shape)
    # print(mapStat)
...
def GetStep(playerID, mapStat, sheepStat):
    ...
    '''
    # print('*' * 10 + ' get step ' + '*' * 10)
    # print(playerID)
    mapStat = np.array(mapStat, dtype=int)
    sheepStat = np.array(sheepStat, dtype=int)
    # print(mapStat)
    # print(sheepStat)
    move = monte_carlo_tree_search_ucb(mapStat, sheepStat, playerID, num_iterations=500)
    # print(move)
