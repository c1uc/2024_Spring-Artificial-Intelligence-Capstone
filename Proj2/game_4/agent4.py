...
def calculate_team_score(board, player_id):
    if player_id in [1, 3]:
        return calculate_score(board, 1) + calculate_score(board, 3)
    else:
        return calculate_score(board, 2) + calculate_score(board, 4)
...
def random_simulation(board, sheep, depth, player_id, target_player_id, num_players):
    ...
    if not moves:
        return calculate_score(board, player_id) * (1 if player_id % 2 == target_player_id % 2 else -1)
...
def GetStep(playerID, mapStat, sheepStat):
    ...
    move = monte_carlo_tree_search_ucb(mapStat, sheepStat, playerID, num_iterations=1200)
